...
uint8_t phyLinkStatus = 0;    // PHY链路状态标志：0-断开，1-已连接
uint8_t phyStatus = 0;        // PHY寄存器状态值
uint8_t phyLinkCnt = 0;       // 链路检测计数器
uint8_t phySucCnt = 0;        // 成功建立链路的计数器
uint8_t phyPN = PHY_PN_SWITCH_AUTO;  // PHY极性配置：自动/固定
uint8_t TRDetectStep = 0;     // 发送/接收检测步骤状态机
uint8_t TRDetectCnt = 0;      // 发送/接收检测计数器
uint8_t LinkTaskPeriod = 50;  // 链路检测任务周期（ms）
uint32_t RandVal = 0;         // 随机值，用于链路恢复的随机延迟
uint8_t volatile phyLinkReset;  // PHY链路复位标志（volatile，中断中修改）
uint32_t volatile phyLinkTime;   // PHY链路检测时间戳（volatile，中断中修改）

/* PHY receive processing */
uint8_t ReInitMACFlag = 0;     // MAC重新初始化标志
uint8_t PhyPolarityDetect = 0; // PHY极性检测使能标志
uint32_t LinkSuccTime = 0;     // 上一次链路成功建立的时间

/*Parameters used when connecting to
 * devices that do not support auto-negotiation.*/
uint8_t phyNegoStat = 0;       // PHY协商状态：0-未协商，1-协商中，2-协商完成
uint32_t phyNegoLinkTime = 0;  // PHY协商开始时间戳

extern u8 MACAddr[6];          // 外部MAC地址数组（6字节）
void ReInitMACReg(void);       // MAC寄存器重新初始化函数声明
