...
/* LED控制结构体 */
typedef struct {
    u8 current_state;         // 当前LED状态（1=亮，0=灭）
    u8 target_state;          // 目标LED状态
    u32 trigger_time;         // 触发时间戳
    u8 is_blinking;           // 是否在闪烁中（点亮或等待熄灭）
    u32 last_off_time;        // 上次熄灭时间戳（用于控制熄灭后的最小等待时间）
} LED_Control_t;

static LED_Control_t rx_led = {0, 0, 0, 0, 0};  // 接收LED控制
static LED_Control_t tx_led = {0, 0, 0, 0, 0};  // 发送LED控制

/*********************************************************************
 * @fn      NEN_RX_LED_SET
 *
 * @brief   设置网络接收LED指示灯状态（GPIO PA13）
 *
 * @param   Val - 1:点亮LED, 0:熄灭LED
 *
 * @return  none
 */
void NEN_RX_LED_SET(u8 Val)
{
    GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, (BitAction)(Val ? Bit_SET : Bit_RESET));
}

/*********************************************************************
 * @fn      NEN_TX_LED_SET
 *
 * @brief   设置网络发送LED指示灯状态（GPIO PA14）
 *
 * @param   Val - 1:点亮LED, 0:熄灭LED
 *
 * @return  none
...
{
    GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, (BitAction)(Val ? Bit_SET : Bit_RESET));
}

/*********************************************************************
 * @fn      NEN_RX_LED_Toggle
 *
 * @brief   翻转网络接收LED指示灯状态（GPIO PA13）
 *
 * @return  none
 */
void NEN_RX_LED_Toggle(void)
{
    u8 current_state = GPIO_ReadOutputDataBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN);
    GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, (BitAction)((current_state == Bit_RESET) ? Bit_SET : Bit_RESET));
}

/*********************************************************************
 * @fn      NEN_TX_LED_Toggle
 *
 * @brief   翻转网络发送LED指示灯状态（GPIO PA14）
 *
 * @return  none
 */
void NEN_TX_LED_Toggle(void)
{
    u8 current_state = GPIO_ReadOutputDataBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN);
    GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, (BitAction)((current_state == Bit_RESET) ? Bit_SET : Bit_RESET));
}
/*********************************************************************
 * @fn      NEN_RX_LED_SetState
 *
 * @brief   设置接收LED状态（GPIO PA13）
 *
 * @param   state - 1:点亮LED, 0:熄灭LED
 *
 * @return  none
 */
void NEN_RX_LED_SetState(u8 state)
{
    rx_led.current_state = state ? 1 : 0;
    rx_led.target_state = rx_led.current_state;
    rx_led.last_off_time = g_ulSystemTick;       // 记录熄灭时间
    rx_led.is_blinking = 0;
    GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, (BitAction)(state ? Bit_RESET : Bit_SET));  // 低电平点亮
}

/*********************************************************************
 * @fn      NEN_TX_LED_SetState
 *
 * @brief   设置发送LED状态（GPIO PA14）
 *
 * @param   state - 1:点亮LED, 0:熄灭LED
 *
 * @return  none
 */
void NEN_TX_LED_SetState(u8 state)
{
    tx_led.current_state = state ? 1 : 0;
    tx_led.target_state = tx_led.current_state;
    tx_led.last_off_time = g_ulSystemTick;       // 记录熄灭时间
    tx_led.is_blinking = 0;
    GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, (BitAction)(state ? Bit_RESET : Bit_SET));  // 低电平点亮
}

/*********************************************************************
 * @fn      NEN_RX_LED_Trigger
 *
 * @brief   触发接收LED闪烁（点亮后自动熄灭）
 *
 * @note    只有LED熄灭且超过最小等待时间后才能再次点亮
 *
 * @return  none
 */
void NEN_RX_LED_Trigger(void)
{
    // 检查：LED不在闪烁中 且 距离上次熄灭已超过最小等待时间 
    if( rx_led.is_blinking == 0 &&
        (g_ulSystemTick - rx_led.last_off_time) >= LED_OFF_BLINK_TIME )
    {
        rx_led.target_state = 1;  // 点亮LED
        rx_led.trigger_time = g_ulSystemTick;
        rx_led.is_blinking = 1;
        GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, Bit_RESET);  // 立即点亮（低电平）
        //UART_DEBUG("\r\n ---- NEN_RX_LED_Trigger ----\r\n");
    }
}

/*********************************************************************
 * @fn      NEN_TX_LED_Trigger
 *
 * @brief   触发发送LED闪烁（点亮后自动熄灭）
 *
 * @note    只有LED熄灭且超过最小等待时间后才能再次点亮
 *
 * @return  none
 */
void NEN_TX_LED_Trigger(void)
{
    // 检查：LED不在闪烁中 且 距离上次熄灭已超过最小等待时间
    //UART_DEBUG("\r\n NEN_TX is-%d,g_t:%d-last_t:%d----\r\n",tx_led.is_blinking,g_ulSystemTick,tx_led.last_off_time );
    if( tx_led.is_blinking == 0 &&
        (g_ulSystemTick - tx_led.last_off_time) >= LED_OFF_BLINK_TIME )
    {
        tx_led.target_state = 1;  // 点亮LED
        tx_led.trigger_time = g_ulSystemTick;
        tx_led.is_blinking = 1;
        GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, Bit_RESET);  // 立即点亮（低电平）
        //UART_DEBUG("\r\n ---- NEN_TX_LED_Trigger ----\r\n");
    }
}
 ...
 * @fn      NEN_LED_Update
 *
 * @brief   LED定时更新函数，需在主循环中周期调用（建议1ms周期）
 *          功能：自动熄灭闪烁的LED，并记录熄灭时间
 *
 * @return  none
 */
void NEN_LED_Update(void)
{
    // 更新接收LED
    if (rx_led.is_blinking && (g_ulSystemTick - rx_led.trigger_time) >= LED_RX_BLINK_TIME)
    {
        rx_led.is_blinking = 0;
        rx_led.target_state = 0;                     // 熄灭LED
        rx_led.last_off_time = g_ulSystemTick;       // 记录熄灭时间
        GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, Bit_SET);  // 熄灭（高电平）
        //UART_DEBUG("\r\n NEN_RX last time = %d  \r\n",rx_led.last_off_time);
    }

    // 更新发送LED
    if (tx_led.is_blinking && (g_ulSystemTick - tx_led.trigger_time) >= LED_TX_BLINK_TIME)
    {
        tx_led.is_blinking = 0;
        tx_led.target_state = 0;                     // 熄灭LED
        tx_led.last_off_time = g_ulSystemTick;       // 记录熄灭时间
        GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, Bit_SET);  // 熄灭（高电平）
        //UART_DEBUG("\r\n NEN_TX last time = %d  \r\n",tx_led.last_off_time);
    }
}
...
/*********************************************************************
 * @fn      GPIO_NEN_LED_INIT
 *
 * @brief   Initializes GPIOA.0
 *
 * @return  none
 */
void GPIO_NEN_LED_INIT(void)
{
    GPIO_InitTypeDef GPIO_InitStructure = {0};

    RCC_APB2PeriphClockCmd(LED_RX_GPIO_RCC, ENABLE);

    GPIO_InitStructure.GPIO_Pin = LED_RX_GPIO_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(LED_RX_GPIO_TypeDef, &GPIO_InitStructure);

    GPIO_InitStructure.GPIO_Pin = LED_TX_GPIO_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(LED_TX_GPIO_TypeDef, &GPIO_InitStructure);

    GPIO_WriteBit(LED_RX_GPIO_TypeDef, LED_RX_GPIO_PIN, Bit_SET);  // 初始电平 拉高 led熄灭
    GPIO_WriteBit(LED_TX_GPIO_TypeDef, LED_TX_GPIO_PIN, Bit_SET);  // 初始电平 拉高 led熄灭
}
