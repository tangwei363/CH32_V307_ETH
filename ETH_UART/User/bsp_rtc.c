...
* Version            : V1.0.0
* Date               : 2024/07/18
* Description        : bsp_rtc program body.
*********************************************************************************
...
u8 RTC_Init(void)
{
    ...
    /* ±æ∞ÂŒﬁÕ‚æß’Ò°¢ŒﬁRTCµÁ≥ÿ, RTCΩˆ◊˜√Î∂® ±ª˘◊º;
       ’Ê µ ±º‰√ø¥Œ…œµÁ”…PLCœ¬∑¢∏≤∏«, π Œﬁ–Ë" «∑Ò“—≈‰÷√"ºÏ≤‚, √ø¥Œ÷±Ω”(÷ÿ–¬)≥ı ºªØ */
    RCC_LSICmd(ENABLE);                          // ‰ΩøËÉΩÂÜÖÈÉ®‰ΩéÈÄüÊô∂ÊåØ(LSI, Ê†áÁß∞‚âà40kHz)
    ...
    RTC_ITConfig(RTC_IT_SEC, ENABLE);            // 1√Î÷‹∆⁄÷–∂œ
    RTC_WaitForLastTask();

    RTC_EnterConfigMode();
    RTC_SetPrescaler(39999);                     // LSI‚âà40kHz ‚Üí 40k ÂàÜÈ¢ëÂæóÂà∞ 1Hz ÁßíËäÇÊãç
    RTC_WaitForLastTask();
    RTC_Set(2026, 10, 1, 0, 0, 0);               // ≥ı ºª˘◊º(…œµÁ∫Û”…PLC ±º‰∏≤∏«)
    RTC_ExitConfigMode();
...
u8 RTC_Get(void)
{
    ...
    // BSP_DEBUG("get rtc time  %04d-%02d-%02d  %02d:%02d:%02d\r\n",
